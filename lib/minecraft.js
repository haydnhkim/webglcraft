...
const TextureHelper = {
  loadTexture(path) {
    ...
    const map = new Texture(
      image, new UVMapping(), ClampToEdgeWrapping, ClampToEdgeWrapping,
  ...
  tileTexture: function(path, repeatx, repeaty) {
    ...
    image.src = path;
    const map = new Texture(
      image, new UVMapping(), RepeatWrapping, RepeatWrapping, NearestFilter,
...
class Game {
  ...
  generateHeight() {
    ...
    size.times(i => {
      data[i] = [];
      return size.times(j => data[i][j] = 0);
    });
  ...
  loadWorld() {
    ...
    const results = [];
    for (let x = l = 0, len = map.length; l < len; x = ++l) {
      const mapYZ = map[x];
      results.push(() => {
        const results1 = [];
        for (let y = m = 0, len1 = mapYZ.length; m < len1; y = ++m) {
          const mapZ = mapYZ[y];
          results1.push(() => {
            const results2 = [];
            for (let z = n = 0, len2 = mapZ.length; n < len2; z = ++n) {
  ...
  populateWorld() {
    ...
    for (let i = l = -5; l <= 5; i = ++l) {
      for (let j = m = -5; m <= 5; j = ++m) {
        ...
        height.times((() =>
          k => this.cubeAt(middle + i, k, middle + j)
        )());
      }
    }
  ...
  start() {
    const animate = () => {
      ...
      requestAnimationFrame(animate, this.renderer.domElement);
    };
    animate();
  }
